...
-(void)shareWithParams:(id)params
{
    NSMutableDictionary *shareParams = [NSMutableDictionary dictionary];
    NSDictionary *paramsFromWeb;
    if ([params isKindOfClass:[NSString class]]) {
        NSError *error;
        params = [NSJSONSerialization JSONObjectWithData:[params dataUsingEncoding:NSUTF8StringEncoding] options:NSJSONReadingMutableContainers error:&error];
        if (!error) {
            NSLog(@"分享参数传入字符串解析字典错误:%@",error.localizedDescription);
        }
    }
    else if ([params isKindOfClass:[NSDictionary class]]) {
        paramsFromWeb = [NSDictionary dictionaryWithDictionary:params];
    }
    else{
        
    }
    [shareParams SSDKSetupShareParamsByText:[paramsFromWeb objectForKey:@"text"]
                                     images:[paramsFromWeb objectForKey:@"image"]
                                        url:[paramsFromWeb objectForKey:@"url"]
                                      title:[paramsFromWeb objectForKey:@"title"]
                                       type:SSDKContentTypeAuto];
    
    
    UINavigationController *navi = (UINavigationController*)((AppDelegate*)[[UIApplication sharedApplication] delegate]).window.rootViewController;
    UIViewController *vc = navi.topViewController;
    [ShareSDK showShareActionSheet:vc.view
                             items:self.platforems
...
//                           [alert show];
                           [vc.view makeToast:[NSString stringWithFormat:@"分享失败:%@",error.localizedDescription] duration:2 position:CSToastPositionBottom];
                           break;
                       }
